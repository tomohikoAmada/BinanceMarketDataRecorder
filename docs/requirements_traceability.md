...
| Reconnect boundary repair | M21.4.11 PR `fix/m21-4-reconnect-boundary-integrity`: unified state machine, manifest-level `reconnect_gap`, seal defense, read-only audit tool | Implemented; under review; NOT DEPLOYED |
| Historical audit | 160,794 chunks scanned; 4,691 transitions; 11 explicit; 4,680 unmarked | `tools/audit_reconnect_boundaries.py` + `RUN_ROOT/historical-reconnect-audit.json` (SHA-256 92943961...) |
