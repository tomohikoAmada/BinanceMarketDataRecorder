...
| Reconnect boundary repair | M21.4.11 PR `fix/m21-4-reconnect-boundary-integrity`: unified state machine, manifest-level `reconnect_gap`, seal defense, read-only audit tool; M21.4.11-R1..R5 correction: crash-durable STARTED-before-seal ordering, boundary-local classification, strictly read-only audit, side-data fail-closed restart, deterministic canonical audit output | Implemented; under review; NOT DEPLOYED |
| Historical audit (corrected, authoritative) | 161,817 chunks scanned (cutoff `1786349202047196027`); 4,691 transitions; 11 explicit (all Catalog-identity-proven); 4,680 unmarked; 0 unknown/ambiguous; catalog matched_pairs=11/unmatched=0/0 | `tools/audit_reconnect_boundaries.py` + `/var/tmp/m21-4-11-corrected-historical-audit.json` (canonical SHA-256 7143bc0c...); earlier 4,691/11/4,680 figures from the manifest-level scanner retained as SUPERSEDED_BY_CORRECTED_BOUNDARY_LOCAL_AUDIT |
