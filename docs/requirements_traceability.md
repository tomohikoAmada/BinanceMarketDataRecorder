...
| FAI-06 | Future exact-VPS staged acceptance is artifact-bound, reboot-sensitive, and fail-closed on evidence loss | M22.9 acceptance observer, Risk R-055/R-070/R-071 | The corrected exact source `e267ae38…` produced one valid retry T0 and ten samples, then host maintenance broke Recorder/observer identity before finalization. The retry is incomplete with zero credit. The 2026-09-13 restart-from-scratch maintenance preflight completed pending packages, proved the runtime-only apt/unattended exclusion, restored update authority, and created no T0. One new 2h retry remains next after fresh pre-start gates |
...
| VPS-01 | Ubuntu 24.04 LTS x86_64, Python 3.12, non-root systemd, shared engineering VPS | ADR-0028 | Exact source `e267ae38…` remains installed on Ubuntu 24.04.4/kernel 6.8.0-139. Recorder is inactive/disabled and the archive timer is enabled/active. Formal retry credit is zero; 12h and Production Ready remain not started/not authorized |
