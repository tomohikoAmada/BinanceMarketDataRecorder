...
| CFG-01 | Operator-configured finite independent Spot/USD-M symbol lists; uppercase boundary canonicalization; duplicate/invalid rejection; legacy BTC/BTC defaults only when both fields are absent; explicit mode resolves an omitted sibling empty; restart-only topology changes; no discovery/env/CLI product DSL | ADR-0032 | MS2 PASS: `test_ms2_configurable_products.py`; TOML presence, validation, restart and topology |
| CFG-02 | ProductKey `(market, symbol)` runtime ownership; one process/Catalog; per-product isolation; shared Spot limiter and USD-M REST authority; conditional process-global USD-M side-data owner; exact config-bound readiness; zero-product markets produce zero market traffic | ADR-0032, architecture | MS2 PASS; MS3-B merged through PR #56 with model fairness, real Collector/Poller gate, finite pagination/cursor, cancel/stop, mixed running-path, archive/capacity, and side-data isolation evidence; MS4-B stopped deployment review complete; the original MS4-C two-hour window observed exact four-ProductKey readiness but remains partial; the separate R3 non-formal supplement passed 15 post-fault all-product-ready observations and 12-context sealed/Catalog validation, closing the recovery gate for review; MS4-D and overall MS4 are `REVIEWED_COMPLETE` for `NONFORMAL_BOUNDED_FOUR_PRODUCT_CORE`; no automatic all-symbol discovery or Formal M22.9 claim |
...
| FAI-02 | 72-hour and 168-hour continuous-operation proof with resource/rotation/archive evidence | Risk R-035 and Future Work | Exact installed source `e267ae38…` has zero duration credit: one Formal 2h retry was maintenance-interrupted and the quiet-window retry finalized `INCOMPLETE` on observer evidence cadence despite stable Recorder identity. The pre-MS1 clean 24h non-formal stage remains artifact-specific; 12h/24h/72h/168h are not started |
...
| FAI-06 | Future exact-VPS staged acceptance is artifact-bound, reboot-sensitive, bounded-memory, and fail-closed on evidence loss | M22.9 acceptance observer, Risk R-055/R-070/R-071/R-074 | The exact source `e267ae38…` produced one maintenance-interrupted retry and then a quiet-window final with stable Recorder identity. The latter is still incomplete with zero credit because approximately 223 MB full-state samples exceeded the 600-second evidence cadence. Repeated evidence and completed-chain verification require a bounded-memory fix before another exact-artifact deploy and 2h retry |
| FAI-07 | Startup recovery remains live and stoppable without false completion | Risk R-056/R-073, M22.9 current-state record | A later host reboot auto-started the enabled exact deployment for about six minutes; it stopped successfully, created no observer/stage, and receives no credit. Recorder is now inactive and disabled while the archive timer remains enabled/active |
| OPS-03 | Remote VPS administration cannot broaden cleanup through an unset or cross-SSH variable, and stopped handoff cannot silently arm reboot start | VPS operations, Risk R-072/R-073 | The 2026-09-12 root-home incident is recovered and recorded. Cleanup defaults to retained/fresh workspaces and requires same-shell canonical exact-child guards when unavoidable. Current Recorder is inactive/disabled; future enable/start is explicit |
| VPS-01 | Ubuntu 24.04 LTS x86_64, Python 3.12, non-root systemd, shared engineering VPS | ADR-0028 | Exact source `e267ae38…` remains installed on Ubuntu 24.04.4/kernel 6.8.0-139. Recorder is inactive/disabled, archive backlog/pending are zero, and the archive timer is enabled/active. Quiet-window Formal retry credit is zero; 12h and Production Ready remain not started/not authorized |
...
| ARC-03 | Archive Set logical identity, physical storage_id, whole-chunk placement, self-describing media, rebuildable index | ADR-0030, offline workspace | Archive Set/library support, rebuild tests, and the isolated cross-machine matrix are implemented and accepted; only the operator-selected production archive-machine workflow, target execution, and certification remain pending |
| ARC-04 | Post-session consistent Catalog snapshots with latest+previous retention; snapshots never replace Raw | ADR-0029, offline workspace, R-050 | Snapshot protocol/library support, implementation/tests, and the isolated cross-machine matrix are implemented and accepted; only the operator-selected production archive-machine workflow, target execution, and certification remain pending |
| CAP-04 | VPS free-byte/ETA policy: 18/14/12/10 GiB and 7d/72h/24h, protected 10 GiB reserve | ADR-0028, vps_operations, R-049 | Thresholds are unchanged. MS3-B offline evidence preserves aggregate growth, topology-decomposition invariance, global state, and the 42-identity hard-reserve stop. Bounded MS4-C/R3 evidence recorded minimum free bytes `44328247296`, zero hard-reserve violations, and last hard-reserve ETA about `25 h` versus the `3 h` envelope; forecast states also included WARNING/CRITICAL/EMERGENCY and shared-host changes were not attributable. This is bounded capacity evidence with scope limits, not Formal M22.9 runway proof; the long formal chain remains unstarted |
...
the 168h window was not run. Those are historical artifact results. The PR #11
corrections are included in the recorded older deployed source `e074d41a…`, which has a
separate 2h non-formal M23.4 pass but no formal M22.9 duration credit.
