...
| Spot exchangeInfo | startup/hourly, configurable | not imported | receive time plus serverTime when supplied | filters/status/order types |
...
| USD-M mark price/liquidation/funding/OI/exchange info | configurable independent tasks | fundingRate baseline | liquidation is event-sparse; no forward fill | derivatives monitoring/research |
| USD-M OI history, taker volume, global/top ratios, basis | latest closed 5m, configurable | not in baseline archive | official latest month/30-day retention; capture continuously | crowding, flow, leverage and basis features |
