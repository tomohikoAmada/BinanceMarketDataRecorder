...
lineage. Downloads use `.partial`, bounded one-file concurrency, safe Range
resume, fsync, checksum verification, then atomic commit. A 404 creates an
explicit `historical-gap.v1`.
...
Parquet embeds source revision and ZIP SHA-256. Its clock is
