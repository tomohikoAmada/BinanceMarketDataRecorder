...
| R-034 | Current official Global Spot bootstrap wording conflicts with the official toolbox example and observed adjacent snapshot/diff boundary | Critical | Preserve Raw evidence; ADR-0011 versions Spot as `lastUpdateId + 1` based on toolbox commit plus observed `[L+1,...]`, while explicitly not claiming the Global text is corrected. Unpublished issue draft seeks confirmation. ADR-0022 adds single-flight/process-shared pacing, 429/418 containment and default weight-50 snapshots. Revert condition is a corrected normative source/maintainer answer or repeatable book-quality divergence; then stop readiness and version/rebuild derived data from Raw. | M17 | Open |
